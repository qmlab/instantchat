...
$(".callStatus").show()};x.onAudioStreamclose=function(){$(".audioIcon").hide();$(".stopAudio").hide();$(".mediaControls").hide();$(".callStatus").hide()};$("#videoChannel").on("hidden.bs.modal",function(){x.inSession&&($(".videoIcon").show(),$(".videoIcon").popover({title:m("Reminder"),content:"<p>"+m("Click here to reopen the hidden video pane")+"</p>",html:!0,trigger:"manual"}),$(".videoIcon").popover("show"),setTimeout(function(){$(".videoIcon").popover("hide")},3E3))});var Q=Common.getCookie("username"),
w=Common.getCookie("roomname"),ba=null,da=Common.getCookie("authInfo");0<da.length&&(ba=JSON.parse(da));w&&""!==w&&e.val(w);0<Q.length&&0<w.length&&"undefined"!==Q&&ba?N(Q,w,ba):g.show();z.emit("get ip");z.on("return ip",function(a){var b=a.split("."),d=(new Date).getTime()%60;if(!(4<b.length)){a="";for(var c=0;4>c;c++){var q=parseInt(b[c]).toString(16);a+=q}}B="Guest_"+a+d});$("#enterRoomGuest").click(function(a){t=null;Common.deleteCookie("username");Common.deleteCookie("authInfo");N(B,Common.cleanInput(e.val().trim()))});
$("#enterRoomFacebook").click(function(a){FB.login(function(a){"connected"===a.status?FB.api("/me",function(b){console.log("Successful login for: "+b.name);t=b.name;var d=a.authResponse;d.type="facebook";N(Common.cleanInput(b.name),Common.cleanInput(e.val().trim()),d)}):("not_authorized"===a.status&&bootbox.alert(m("Error")+":"+m("Failed to login")),H=t=null)})});$(".roomnameInput").keydown(function(a){13===a.which&&N(t||B,Common.cleanInput(e.val().trim()))});c.keydown(function(a){r.hide()});a.keydown(W);
b.keydown(W);a.on("input",function(){1<a.val().length&&ca()});a.click(function(){a.focus()});b.click(function(){b.focus()});z.on("disconnect",function(){bootbox.confirm(m("You are disconnected. Do you want to re-login?"),function(a){a?window.location.reload():(window.open("","_parent",""),window.close())})});z.on("logged out",function(a){bootbox.alert(m("You are logged out")+". "+m(a.msg),function(){Common.deleteCookie("username");Common.deleteCookie("roomname");Common.deleteCookie("authInfo");window.location.reload(!0)})});
z.on("logged in",function(b){V=!0;g.fadeOut();k.show();a.focus();t=b.username;F=b.roomname;supportRTC()||n(m("[Please use Chrome 29+ or Firefox 38+ for video/audio chats and file transfer]"),{color:"orange"});var c=m("Welcome ")+b.username+m(" to Room ")+'"'+b.roomname+'"';n(c);R(b);b.users.forEach(function(a,b,c){K(a,d)});t&&"undefined"!==t&&""!==t&&Common.setCookie("username",t,1);F&&""!==F&&Common.setCookie("roomname",F,7);H&&t&&Common.setCookie("authInfo",JSON.stringify(H),1)});z.on("new message",
function(a){M(a)});z.on("new info",function(a){n(a.message)});z.on("new poke",function(a){a=a.username+m(" has poked you!");n(a);alert(a)});z.on("user joined",function(a){n(a.username+m(" has joined"));K(a.username,d)});z.on("user left",function(a){n(a.username+m(" has left"));J(a);$('.users > li:contains("'+a.username+'")').remove()});z.on("typing",function(a){a.typing=!0;a.message=m("is typing");M(a)});z.on("stop typing",function(a){J(a)});z.on("login error",function(a){bootbox.alert(m("Error")+
":"+m(a.msg));t=H=null;g.show()});$("ul.users").on("contextmenu",".username",L);$("ul.users").on("click",".username",L);$("ul.users").on("dragover",".username",P);$("ul.users").on("dragenter",".username",P);$("ul.users").on("drop",".username",X);$("ul.messages").on("contextmenu",".username",L);$("ul.messages").on("click",".username",L);$("ul.messages").on("dragover",".username",P);$("ul.messages").on("drop",".username",X);$("#sendMsg").click(function(a){a=r.data("toUser");l.data("toUser",a);l.find(".modal-title").text("To "+
a);l.modal("toggle")});$("#startVideo").click(function(a){supportRTC()?(a=r.data("toUser"),n(m("initiating video connection with ")+a),z.emit("start video request",{to:a})):(a.preventDefault(),a.stopPropagation())});$("#startAudio").click(function(a){supportRTC()?(a=r.data("toUser"),n(m("initiating audio connection with ")+a),z.emit("start audio request",{to:a})):(a.preventDefault(),a.stopPropagation())});$("#sendFile").click(function(a){a.preventDefault();a.stopPropagation();supportRTC()&&$("#fileInput").trigger("click")});
$("#fileInput").change(function(a){var b=r.data("toUser");Y[b]=a.target.files;n(m("initiating file transfer with ")+b);z.emit("start file request",{to:b})});$(window).scroll(function(){Common.isScrolledToTop()?$(".scrollArrow.upArrow").css("opacity",.5):$(".scrollArrow.upArrow").css("opacity",1);Common.isScrolledToBottom()?$(".scrollArrow.downArrow").css("opacity",.5):$(".scrollArrow.downArrow").css("opacity",1)});$(".scrollArrow.upArrow").click(function(){$("body,html").animate({scrollTop:0},500)});
$(".scrollArrow.downArrow").click(function(){$("body,html").animate({scrollTop:$(".messages")[0].scrollHeight},500)});z.on("start video response",function(a){if(a&&a.to){var b=a.to;a.permitted?(G(b,(t||B)+m("has initiated a video chat")),x.startVideo(b,t||B)):n(m("failed to start video with ")+b+". "+a.message)}});z.on("start audio response",function(a){if(a&&a.to){var b=a.to;a.permitted?(G(b,(t||B)+m("has initiated an audio chat")),x.startAudio(b,t||B)):n(m("failed to start audio with ")+b+". "+
a.message)}});z.on("start file response",function(a){if(a&&a.to){var b=a.to;if(a.permitted){var d=Y[a.to];delete Y[a.to];p(d,b)}else n(m("failed to start file transfer with ")+b+". "+a.message)}});$("#poke").click(function(a){a=r.data("toUser");5E3<Date.now()-Z?(n(m("You poked ")+a),z.emit("new poke",{toUser:a}),Z=Date.now()):bootbox.alert(m("Sorry, you are poking too fast!"))});$("#sendPrivateMsgBtn").click(function(a){E(l.data("toUser"))});l.on("shown.bs.modal",function(){b.focus()});$("body").on("click",
function(){r.hide()});$("body").mouseover(function(){q.isCancelled=!0;$(document).prop("title",T)});$(".navbarItem").click(function(a){$(".navbar-toggle").is(":visible")&&$(".navbar-collapse").collapse("hide")});$("#listroommates").click(function(a){U()});$("#about").click(function(a){bootbox.dialog({message:m("About Content"),title:m("About TalkYet"),onEscape:function(){},show:!0,buttons:{success:{label:"OK",className:"btn-success",callback:function(){}}}})});$("#quit").click(function(a){bootbox.confirm(m("Are you sure to quit?"),
function(a){!0===a&&(Common.deleteCookie("username"),Common.deleteCookie("roomname"),Common.deleteCookie("authInfo"),window.location.reload(!0))})});$(".stopVideo").click(function(a){if(a=x.getPeer())G(a,(t||B)+m("has stopped video chat")),x.stopVideo(),$(".mute").attr("checked",!0)});$(".stopAudio").click(function(a){if(a=x.getPeer())G(a,(t||B)+m("has stopped audio chat")),x.stopAudio(),$(".mute").attr("checked",!0)});$(".mute").on("switchChange.bootstrapSwitch",function(a,b){$(".mute").attr("checked",
b);var d=x.getPeer();d&&(b?G(d,(t||B)+m("has unmuted their mic")):G(d,(t||B)+m("has muted their mic")),x.muteMe(b))});$(".remoteVideo").dblclick(function(a){document.fullscreenElement||document.mozFullScreenElement||document.webkitFullscreenElement||document.msFullscreenElement?document.exitFullscreen?document.exitFullscreen():document.msExitFullscreen?document.msExitFullscreen():document.mozCancelFullScreen?document.mozCancelFullScreen():document.webkitExitFullscreen&&document.webkitExitFullscreen():
