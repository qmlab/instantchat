...
a.toUser&&F.addClass("private");a.username===(t||B)?H.addClass("me").append(e,d,F):H.addClass("you").append(F,e,d);b.scrollToBottom=!0;Common.addElement(H,f,c,b);Z&&window.clearTimeout(Z);Z=window.setTimeout(function(){n(m("last message at")+" "+Common.getCurrentTime())},12E4);a.username===(t||B)||a.typing||(q.isCancelled=!1,Common.newMsgTitle(m("New messages"),q))}function U(a){a=a||{};a.scrollToBottom=!0;var b=$('<li class="message"/>'),d=$('<span class="log"/>').text(m("Current people in the room")+
":");b.html(d);$("ul.users li").each(function(){var a=$('<span class="username"/>').text($(this).text()).css("color",Common.getUsernameColor($(this).text()));b=b.append(a)});Common.addElement(b,f,c,a)}function J(a){h(a).fadeOut(function(){$(this).remove()})}function ba(){V&&(C||(C=!0,z.emit("typing")),O=(new Date).getTime(),setTimeout(function(){2E3<=(new Date).getTime()-O&&C&&(z.emit("stop typing"),C=!1)},2E3))}function h(a){return $(".typing.message").filter(function(b){return $(this).data("username")===
...
$(".callStatus").show()};x.onAudioStreamclose=function(){$(".audioIcon").hide();$(".stopAudio").hide();$(".mediaControls").hide();$(".callStatus").hide()};$("#videoChannel").on("hidden.bs.modal",function(){x.inSession&&$(".videoIcon").show()});var Q=Common.getCookie("username"),w=Common.getCookie("roomname"),aa=null,da=Common.getCookie("authInfo");0<da.length&&(aa=JSON.parse(da));w&&""!==w&&e.val(w);0<Q.length&&0<w.length&&"undefined"!==Q&&aa?N(Q,w,aa):g.show();z.emit("get ip");z.on("return ip",function(a){var b=
...
Common.cleanInput(e.val().trim()),d)}):("not_authorized"===a.status&&bootbox.alert(m("Error")+":"+m("Failed to login")),H=t=null)})});$(".roomnameInput").keydown(function(a){13===a.which&&N(t||B,Common.cleanInput(e.val().trim()))});c.keydown(function(a){r.hide()});a.keydown(W);b.keydown(W);a.on("input",function(){1<a.val().length&&ba()});a.click(function(){a.focus()});b.click(function(){b.focus()});z.on("disconnect",function(){bootbox.confirm(m("You are disconnected. Do you want to re-login?"),function(a){a?
window.location.reload():(window.open("","_parent",""),window.close())})});z.on("logged out",function(a){Common.deleteCookie("username");Common.deleteCookie("roomname");Common.deleteCookie("authInfo");window.location.reload(!0);bootbox.alert(m("You are logged out")+". "+m(a.msg))});z.on("logged in",function(b){V=!0;g.fadeOut();k.show();a.focus();t=b.username;F=b.roomname;supportRTC()||n(m("[Please use Chrome 29+ or Firefox 38+ for video/audio chats and file transfer]"),{color:"orange"});var c=m("Welcome ")+
b.username+m(" to Room ")+'"'+b.roomname+'"';n(c);R(b);b.users.forEach(function(a,b,c){K(a,d)});t&&"undefined"!==t&&""!==t&&Common.setCookie("username",t,1);F&&""!==F&&Common.setCookie("roomname",F,7);H&&t&&Common.setCookie("authInfo",JSON.stringify(H),1)});z.on("new message",function(a){M(a)});z.on("new info",function(a){n(a.message)});z.on("new poke",function(a){a=a.username+m(" has poked you!");n(a);alert(a)});z.on("user joined",function(a){n(a.username+m(" has joined"));K(a.username,d)});z.on("user left",
function(a){n(a.username+m(" has left"));J(a);$('.users > li:contains("'+a.username+'")').remove()});z.on("typing",function(a){a.typing=!0;a.message=m("is typing");M(a)});z.on("stop typing",function(a){J(a)});z.on("login error",function(a){bootbox.alert(m("Error")+":"+m(a.msg));t=H=null;g.show()});$("ul.users").on("contextmenu",".username",L);$("ul.users").on("click",".username",L);$("ul.users").on("dragover",".username",P);$("ul.users").on("dragenter",".username",P);$("ul.users").on("drop",".username",
X);$("ul.messages").on("contextmenu",".username",L);$("ul.messages").on("click",".username",L);$("ul.messages").on("dragover",".username",P);$("ul.messages").on("drop",".username",X);$("#sendMsg").click(function(a){a=r.data("toUser");l.data("toUser",a);l.find(".modal-title").text("To "+a);l.modal("toggle")});$("#startVideo").click(function(a){supportRTC()?(a=r.data("toUser"),n(m("initiating video connection with ")+a),z.emit("start video request",{to:a})):(a.preventDefault(),a.stopPropagation())});
...
e(c[d],/:(\d+) opus\/48000/i);if(b){for(var d=c,g=a,k=c[a].split(" "),r=[],l=0,u=0;u<k.length;u++)3===l&&(r[l++]=b),k[u]!==b&&(r[l++]=k[u]);b=r.join(" ");d[g]=b}break}d=c[a].split(" ");for(g=c.length-1;0<=g;g--)if(b=e(c[g],/a=rtpmap:(\d+) CN\/\d+/i))b=d.indexOf(b),-1!==b&&d.splice(b,1),c.splice(g,1);c[a]=d.join(" ")}return c=c.join("\r\n")},setSDPBandwidth:function(c,d){for(var a=c.split("\r\n"),b=0;b<a.length;b++)a[b]=a[b].replace(/^b=AS:(\d)+/i,"b=AS:"+d);return c=a.join("\r\n")},saveToDisk:function(c,
...
for(d=0<=c?Math.min(c,d-1):d-Math.abs(c);0<=d;d--)if(d in b&&b[d]===a)return d;return-1});"function"!=typeof String.prototype.trim&&(String.prototype.trim=function(){return this.replace(/^\s+|\s+$/g,"")});var E,n=this.jQuery||this.Zepto,y={},v={},K=0,M=[],U=!1,J={};if("undefined"!=typeof module&&module.exports){if(!n)try{n=require("jquery")}catch(ba){}n&&(n.i18n=n.i18n||y);module.exports=y}else n&&(n.i18n=n.i18n||y),this.i18n=this.i18n||y;var J={load:function(a,b,d){b.useLocalStorage?J._loadLocal(a,
